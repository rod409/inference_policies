...
|Single stream |LoadGen sends next query as soon as SUT completes the previous query 
|Constant stream |LoadGen sends new queries to the SUT at a fixed cadence
|===
...
|SSD	          |one image
|DeepLabv3+    |one image
|BEVFormer	    |six images
|UniAD         |six images
|Llama-3.1-8B	 |one prompt
|===
...
The automotive benchmark suite defines tasks and requirements for the closed division. Submitters may choose which benchmarks and scenarios to submit.

Benchmarks:

|===
|Area |Task |Model |Dataset |QSL Size |Quality 
|Perception |2D object detection |SSD |Cognata | 128 | 99.9% of FP32 (0.7179837  mAP) 
|Perception |2D semantic segmentation |DeepLabv3+ |Cognata | 128 | 99.9% of FP32 (0.924355 mIOU)
|Perception |3D object detection |Bevformer |NuScenes | 256 | 99% of FP32 (0.2683556 mAP)
|end-to-end |Path planning | UniAD |NuScenes | 256| 99% of FP32 (0.8959 L2 Avg.)
|IVI |LLM Q/A| LLama-3.1-8B | MMLU | 8192 | 95% of FP16 (64.96% correct answers)
|===
...
each benchmark is listed as 'QSL Size' in the table above. If the tasks has a temporal dependency, such as 3D detection with BEVFormer, LoadGen will select randomly from groups of temporally connected samples defined by the dataset. The accuracy
...
Submitters need to choose different submission categories for their systems. These categories are defined by how close the system is to be in use for a production vehicle. During the review period submissions will be validated for their submission category.

=== Hardened System

These systems are intended for production and have evidence of a straightforward path to meet automotive requirements to be deployed in vehicles, on public roads. These systems are often referred to as an Electronic Control Unit (ECU) which is a general term for a computer inside a production vehicle. MLPerf Automotive does not require submitters to go through software safety certification processes for the benchmark. However, sufficient evidence of certification or for a path to certification is required.

There are three Hardened System Profiles and each the benchmark in the MLPerf Automotive benchmark suite is associated with one or more profiles. 

[cols="2,2,2,2,3"]
|===
.2+h| Hardened System Profile
.2+h| Description
2+h| Evidence
.2+h| Anticipated ML Stack Production Safety Goal
...
| Voice assistant and in vehicle AI experience
| Security manual
| AEC-Q100
| Typically QM, i.e., no safety goal.

| ADAS +
(Advanced Driver Assistance System)
| SAE L0, L1, L2, L2+, L2++ and L2+++
| Security manual +
Safety manual* +
Tool qualification
| AEC-Q100 +
High safety integrity compute
| Typically QM to ASIL-B depending on safety concept.
...
Tool qualification
| AEC-Q100 +
High safety integrity compute with availability
| Typically ASIL-B and above at system level.
|===

=== Development System

Known as EVM (EValuation Model) and is generally what SiPs provide.

=== Engineering Sample 

Very early silicon, internal SoC R&D.
